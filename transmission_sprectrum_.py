...
MOLECULES = {
	'1': [(1.4, 0.05, 1.4), (1.97, 0.05, 1.0), (2.57, 0.07, 1.5), (1.85, 0.05, 0.97), (0.95, 0.045, 0.45), (1.17, 0.045, 0.45), (3.35, 0.06, 0.9), (3.2, 0.06, 0.9), (3, 0.1, 1)],
	'2': [(2.3, 0.1, 0.2), (2.5, 0.01, 0.3), (3.5, 0.2, 1.05), (3.8, 0.03, 0.3)],

	'3': [(2.7, 0.04, 2.1), (2.83, 0.07, 2.25), (4.38, 0.1, 3.7)],
	'4': [(0.1, 0.01, 0.3), (0.7, 0.01, 0.8)],
	'5': [(0.61, 0.03, 0.9)],
	# 'Na/K': [(0.59, 0.01, 0.3), (0.77, 0.01, 0.3)],
	'6': [(0.76, 0.01, 0.8)],
	'7': [(4.03, 0.05, 0.9)],
	"8": [(4.79, 0.3, 1.5), (2.36, 0.1, 0.8)],
	'9': [(4.79, 0.1, 2)]
...
noise = noise_level * np.random.normal(size=len(wavelengths))
# Generate spectrum using Gaussian absorption for each molecule
def generate_spectrum(wavelengths, log_vmr, noise):
	
	spectrum = np.ones_like(wavelengths) + noise
...
def update_plot( **vals):
	...
	wl, spec, spec0 = generate_spectrum(wavelengths, log_vmr, noise)
...
def part_1():
	...
	sliders = {
	    ...
	    for mol in ['1', '2', '3', '4', '5', '6', '7', '8', '9'] #'O2']
	}
...
def part_2(csv):
	...
	slider_box = VBox([sliders[mol] for mol in sliders],layout=Layout(margin='100px 0px 0 0'))
	out = interactive_output(update_plot_2, sliders)
	layout = HBox([out, slider_box],layout=Layout(gap='0px', padding='0px', margin='0px') )
	return display(layout)
